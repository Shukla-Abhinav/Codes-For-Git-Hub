...
def generate_bill(): 
    ...
    quantity=quantityVar.get() 
    cost = costVar.get() 
    conn = pymysql.connect(host="localhost", user="root", password="abhinav", db="python") 
    cursor = conn.cursor() 
...
def print_bill(): 
    ...
    if billFile is None: 
        ...
        billFile.close() 
    #messagebox.showinfo('Bill', message=billString)   
    print(billString) 
...
def loginWindow(): 
    window.configure(bg = '#96afd9')
    titleLabel = Label(window,text="Abhinav Bill Services",font=("Arial", 40,'bold' ),fg="green") 
...
def mainwindow(): 
    ...
    titleLabel = Label(window,text="Abhinav Bill Service",font="Arial 30",fg="green") 
...
def itemAddWindow(): 
    ...
    titleLabel = Label(window,text="Abhinav Bill Service", width=40,font="Arial 30",fg="green") 
